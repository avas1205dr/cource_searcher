...
class StepikParser:
    ...
    async def save_courses(
        self, courses: List[Dict], category_name: str, processed_ids: set
    ) -> int:
        ...
        for i, course in enumerate(new_courses, 1):
            ...
            if i % 10 == 0 or i == len(new_courses):
                print(
                    f"{category_name}: {i}/{len(new_courses)} курсов обработано"
                )
            ...
            for list_name, info in courses_by_lists.items():
                ...
                if category_courses:
                    processed = await self.save_courses(
                        category_courses, list_name, processed_course_ids
                    )
